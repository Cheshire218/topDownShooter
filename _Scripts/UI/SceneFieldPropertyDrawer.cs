	...
	public class SceneFieldPropertyDrawer : PropertyDrawer
	{
		...
		GUIContent label)
		{
			...
			if (sceneAsset != null)
			{
				...
				var value = EditorGUI.ObjectField(position,
				...
				if (EditorGUI.EndChangeCheck())
				{
					...
					{
						var scenePath =
						AssetDatabase.GetAssetPath(sceneAsset.objectReferenceValue);
						var assetsIndex = scenePath.IndexOf("Assets",
						StringComparison.Ordinal) + 7;
						var extensionIndex =
						scenePath.LastIndexOf(".unity", StringComparison.Ordinal);
						scenePath = scenePath.Substring(assetsIndex,
						extensionIndex - assetsIndex);
